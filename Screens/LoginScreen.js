...
const PassengerScreen = () => {
  ...
  const [code, setCode] = useState('');
  const [verificationId, setVerificationId] = useState(null);
  const [loadingGenerateOTP, setLoadingGenerateOTP] = useState(false); // State for Generate OTP loading
  const [loadingVerifyCode, setLoadingVerifyCode] = useState(false); // State for Verify and Confirm loading
  const recaptchaVerifier = React.useRef(null);
  const navigation = useNavigation();
  const [allContactNumbers, setAllContactNumbers] = useState([]);
  ...
  const sendVerification = async () => {
    setCode(''); // Reset the code state
    setLoadingGenerateOTP(true); // Start loading Generate OTP
  ...
  const confirmCode = async () => {
    console.log('Entered phone number:', phoneNumber);
    setLoadingVerifyCode(true); // Start loading Verify and Confirm
    ...
    setLoadingVerifyCode(false); // Stop loading Verify and Confirm
  };
...
<TextInput
  ...
  style={styles.textInput}
  maxLength={13}
  value={phoneNumber || '+639'} // Set initial value to "+639"
/>

      <TouchableOpacity
        style={styles.sendVerification}
        onPress={sendVerification}
        disabled={phoneNumber.trim() === '' || loadingGenerateOTP} // Disable when loading
      >
        {loadingGenerateOTP ? (
          <ActivityIndicator color="#072541" />
        ) : (
          <Text style={styles.buttonText}>Generate OTP</Text>
        )}
      </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        onPress={confirmCode}
        disabled={phoneNumber.trim() === '' || code.trim() === '' || loadingVerifyCode} // Disable when loading
      >
        {loadingVerifyCode ? (
          <ActivityIndicator color="#072541" />
        ) : (
