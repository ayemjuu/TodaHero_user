...
//           <View>
//             <Text style={styles.details}>Driver Name: {historyItem.driverName}</Text>
//             <Text style={styles.details}>Driver TODA Number: {historyItem.driverPlateNumber}</Text>
//             <Text style={styles.details}>Drop-off Point: {historyItem.dropOffPoint}</Text>
//             <Text style={styles.details}>Pickup Point: {historyItem.pickupPoint}</Text>
//             <Text style={styles.details}>Requested By: {historyItem.requestBy}</Text>
//             <Text style={styles.details}>Time Requested: {historyItem.timeRequested}</Text>
...
const HistoryDetailScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchHistoryItem = async () => {
      try {
        ...
        if (doc.exists) {
          ...
          console.log('Time Requested:', data.timeRequested); // Log time requested
          console.log('Time Accepted:', data.timeAccepted); // Log time accepted
    ...
    return () => backHandler.remove(); // Remove event listener when component unmounts
  }, [itemId, navigation]);
  ...
  //   } else if (timestamp instanceof Date) {
  //     // JavaScript Date object
  //     return timestamp.toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true });
  //   } else {
  //     // Some other format
  //     return String(timestamp); // Convert to string
  //   }
  ...
  const formatTimestamp = timestamp => {
    ...
    if (timestamp.toDate) {
      // Firestore Timestamp object
      const date = timestamp.toDate();
      return date.toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }) + " at " + date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
    } else if (timestamp instanceof Date) {
      // JavaScript Date object
      return timestamp.toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }) + " at " + timestamp.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
    } else {
      // Some other format
      return String(timestamp); // Convert to string
    }
      ...
      <TouchableOpacity onPress={() => navigation.navigate('History')} style={styles.backButton}>
        ...
        {historyItem && (
          <View>
            ...
            <Text style={styles.details}>Ride Ended: {formatTimestamp(historyItem.rideEnded)}</Text>
            {/* <Text style={styles.details}>Report: {historyItem.report}</Text> */}
            {historyItem && historyItem.report && (
  <Text style={styles.details}>Report: {historyItem.report}</Text>
)}
