...
const TricycleScreen = () => {
  ...
  const handlePasakay = () => {
    ...
    // .then(() => {
    //   // After successfully saving to "BookNow" collection, also save to "booking" collection
    ...
    .then((docRef) => {
      // Get the auto-generated ID of the document added to "BookNow" collection
      const docId = docRef.id;
    
      // Use the same ID to save to the "booking" collection
      return firebase.firestore().collection('booking').doc(docId).set({
        plateNumber,
        pickupPoint,
        dropOffPoint,
        userName, // Add user's name to the document
    ...
    <View style={styles.container}>
      ...
      <View style={styles.seccontainer}>
        ...
        <Text style={styles.textDriver}>{`TODA Number: ${plateNumber}`}</Text>
            ...
            <TouchableOpacity style={styles.button} onPress={handleMyBookings}>
...
//debugging, yung may phone number na
