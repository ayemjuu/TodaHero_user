...
//backhandler
//working tho time stamp yung time accepted, papalitan sana as string 3-11-24

// import React, { useState, useEffect } from 'react';
// import { View, Text, StyleSheet, Image, TouchableOpacity, Alert, ActivityIndicator, BackHandler } from 'react-native';
// import { firebase } from '../../config';
// import AsyncStorage from '@react-native-async-storage/async-storage';
// import { useNavigation } from '@react-navigation/native';
...
//string conversion og time accepted
    ...
    const fetchRequestData = async () => {
      try {
        ...
        if (doc.exists) {
          ...
          setRequestData({
            ...requestData,
            timeAccepted: formatTimestamp(requestData.timeAccepted),
            timeRequested: formatTimestamp(requestData.timeRequested)
          });
          if (requestData.successful === undefined) {
            setSuccessfulButtonClicked(false);
          } else {
            setSuccessfulButtonClicked(true);
          }

        console.log("Formatted Time Requested:", requestData.timeRequested);

        console.log("Time Accepted:", formatTimestamp(requestData.timeAccepted));
...
const formatTimestampaa = timestamp => {
  if (!timestamp) return null;
  ...
  return date.toLocaleString('en-US', { year: 'numeric', month: 'short', hour: 'numeric', minute: 'numeric', hour12: true });
};
  ...
  //   if (timestamp.toDate) {
  //     const date = timestamp.toDate();
  //     return date.toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) + ' at ' + date.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true });
  //   } else if (timestamp instanceof Date) {
  //     return timestamp.toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) + ' at ' + timestamp.toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true });
  //   } else {
  //     return String(timestamp);
  //   }
  // };
